...
func main() {
	...
	if err != nil {
		fmt.Printf("error: could not connect to game logs queue. - %v\n", err)
		return
	}
	...
	for running {
		...
		case "pause":
			...
			if err != nil {
				fmt.Printf("error: could not publish pause command. - %v\n", err)
				continue
			}
		...
		case "resume":
			...
			if err != nil {
				fmt.Printf("error: could not publish resume command. - %v\n", err)
				continue
			}
